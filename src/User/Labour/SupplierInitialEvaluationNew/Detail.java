...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			pData.pageREQ.mapDynamicData.replace("劳务分包商名录-供应商名称", conName);
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("劳务分包商初评-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("劳务分包商初评-单据编号", pData.pageREQ.mapDynamicData.get("劳务分包商初评-单据编号"));
