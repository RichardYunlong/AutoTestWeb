package User.Labour.SupplierListNew;
...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			//填入【企业名称】
			String conName = "GAT劳务分包商" + GTime.getCurrentTime(GTime.FORMAT_14);
			GWCtrlInputFill.ById(pData.pageREQ.mapContentId.get("企业名称"), conName);
			pData.pageREQ.mapDynamicData.replace("劳务分包商名录-企业名称", conName);
			//填入队长姓名
            String capName = "GAT劳务分包商队长" + GTime.getCurrentTime(GTime.FORMAT_14);
            GWCtrlInputFill.ById(pData.pageREQ.mapContentId.get("队长姓名"), capName);
            pData.pageREQ.mapDynamicData.replace("劳务分包商初评-队长姓名", capName);
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("劳务分包商名录-分包商编码", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("劳务分包商名录-分包商编码", pData.pageREQ.mapDynamicData.get("劳务分包商名录-分包商编码"));
			GScene.DYNAMIC_DATA.replace("劳务分包商名录-企业名称", pData.pageREQ.mapDynamicData.get("劳务分包商名录-企业名称"));
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("劳务分包商名录-分包商编码"));
					break;
				}
				default:{
					break;
