package User.Labour.SupplierEvaluationNew;
...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
            ...
            //填入要评价的【分包商编码】
            GWCtrlPage.ui_C_SELECT_INPUT_BTN(pData.pageREQ.mapContentId.get("分包商名称"), "");
            //游标切换至二级窗体
            ...
            try {
              ...
              treeViewGrid3.ui_C_SELECT_TREEVIEW(pData.pageREQ.mapDynamicData.get("劳务分包商类别-类别编码"));
              //分包商编码
              GWCtrlSelect.ByValue(GWCtrlWebElementId.CN_ID.get("查询方案"), pData.pageREQ.mapDynamicData.get("劳务分包商名录-分包商编码"));
              GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
              sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("劳务分包商名录-分包商编码"));
              //确认
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("劳务分包商评价-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("劳务分包商评价-单据编号", pData.pageREQ.mapDynamicData.get("劳务分包商评价-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("劳务分包商评价-单据编号"));
        	pData.bRES = true;
        	AutoTest.GParam.strTestResultCode = "0000";
            AutoTest.GParam.strTestResultMsg = "OK";
		}catch (Exception e){
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("劳务分包商评价-单据编号"));
					break;
				}
				default:{
					break;
