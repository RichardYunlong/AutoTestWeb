package User.Labour.SupplierConcentrateEvaluationNew;
...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			//填入【评价周期】
        	GWCtrlPage.ui_C_SELECT_INPUT_BTN(pData.pageREQ.mapContentId.get("评价周期"), "");
        	List<WebElement> divSelects = GParam.g_Dr.findElements(By.tagName("span"));
        	if(divSelects != null) {
				for(WebElement divSelect:divSelects){
					if(divSelect.getText().equals("上半年")) {
						divSelect.findElement(By.xpath("..")).click();
					}
				}
			}
	...
	public void ui_C_SP_DETAILS() {
		//设置等待
		pData.pageREQ.MODULE_WAIT_ID = GWCtrlWebElementId.CN_ID.get("供应商列表");
		ui_C_CLICK_TAB("分包商评价");
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("劳务分包商集中评价-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("劳务分包商集中评价-单据编号", pData.pageREQ.mapDynamicData.get("劳务分包商集中评价-单据编号"));
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("劳务分包商初评-单据编号"));
					break;
				}
				default:{
					break;
