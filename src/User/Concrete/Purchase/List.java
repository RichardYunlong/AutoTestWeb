...
import User.Concrete.PurchaseNew.Detail;
...
public class List {
	 ...
	 */
	public void ui_C_BTN_TOPMENU(String menuName, String menuCode){
		...
		try {
			switch(menuName){
				...
				case "删_除":{
					...
					list.ui_C_SEARCH_CODE(menuCode);
					//获取订单状态
					...
					String billState = billView.ui_C_GRID3_VALUE(2, 2).replaceAll(",", "");
			...
			if(!menuCode.equals("")) {
				GLog.logRecordTime(9, "[功能]----对目标[" + menuCode + "]执行[" + menuName + "]成功");
			}
		}catch (Exception e){
			pData.bRES = false;
