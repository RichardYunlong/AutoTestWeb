...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
            ...
            //使用对账单（目前走的逻辑是结算对账单，后续有需求可优化）
            GLog.logRecordTime(9, "[功能]----勾选使用对账单");
            GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
            GWCtrlDivClick.ById("detialTabView");
            GWCtrlInputClick.ById(pData.pageREQ.mapContentId.get("使用对账单"));
	...
	private void ui_C_CLICK_REFER_TO(String strModuelName) {
		...
		try {
			AutoTest.GLog.logRecord(9, "商品混凝土结算单", "res", "选择指定商品混凝土对账单", 8000, 0, 0, "", "微软雅黑", 20, Color.RED);
			GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("选择单据"), "");
			GWCtrlWindow.windowHandles();
			try {
		         //搜索
	            GWCtrlSelect.ByValue(GWCtrlWebElementId.CN_ID.get("查询方案"), pData.pageREQ.mapDynamicData.get("商品混凝土对账单-单据编号"));
	            //选中
	            GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
	            sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("商品混凝土对账单-单据编号"));
	            //移入
	...
	private void ui_C_PARAMS() {
		...
		try {
        	//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("商品混凝土结算单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("商品混凝土结算单-单据编号", pData.pageREQ.mapDynamicData.get("商品混凝土结算单-单据编号"));
			pData.bRES = true;
			AutoTest.GParam.strTestResultCode = "0000";
            AutoTest.GParam.strTestResultMsg = "OK";
			GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("商品混凝土结算单-单据编号"));
		}catch (Exception e){
		    pData.bRES = false;
    		AutoTest.GParam.strTestResultCode = "0000";
            AutoTest.GParam.strTestResultMsg = "OK";
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("商品混凝土结算单-单据编号"));
					break;
				}
				default:{
					break;
