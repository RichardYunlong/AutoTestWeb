package User.Concrete.CheckNew;
...
public class Detail {
	...
	private void ui_C_CLICK_REFER_TO(String strModuelName) {
		...
		try {
			AutoTest.GLog.logRecord(9, "商品混凝土对账单", "res", "选择指定商品混凝土小票", 8000, 0, 0, "", "微软雅黑", 20, Color.RED);
			GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
	...
	private void ui_C_PARAMS() {
		...
		try {
        	//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("商品混凝土对账单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("商品混凝土对账单-单据编号", pData.pageREQ.mapDynamicData.get("商品混凝土对账单-单据编号"));
			pData.bRES = true;
			AutoTest.GParam.strTestResultCode = "0000";
			AutoTest.GParam.strTestResultMsg = "OK";
			GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("商品混凝土对账单-单据编号"));
		}catch (Exception e){
            pData.bRES = false;
            AutoTest.GParam.strTestResultCode = "9999";
            AutoTest.GParam.strTestResultMsg = "ERROR";
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("商品混凝土对账单-单据编号"));
					break;
				}
				default:{
					break;
