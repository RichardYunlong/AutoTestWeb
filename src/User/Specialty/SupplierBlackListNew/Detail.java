package User.Specialty.SupplierBlackListNew;
...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
	        ...
	        try {
              ...
              pData.bRES = true;
              pData.strStatus = "编辑中";
              GLog.logRecordTime(9, "[功能]----选择企业编码成功\n" + JSON.toJSONString(pData.indexRows));
              }catch (Exception e){
              GWCtrlException.SwtichTo(e, 1, 9, "无可被参考的企业编码", true);
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("专业分包商黑名单-分包商编码", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("专业分包商黑名单-分包商编码", pData.pageREQ.mapDynamicData.get("专业分包商黑名单-分包商编码"));
			GScene.DYNAMIC_DATA.replace("专业分包商黑名单-分包商名称", pData.pageREQ.mapDynamicData.get("专业分包商黑名单-分包商名称"));
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("专业分包商黑名单-分包商编码"));
					break;
				}
				default:{
					break;
