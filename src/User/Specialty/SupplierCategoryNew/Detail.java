package User.Specialty.SupplierCategoryNew;
...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			//填入要创建的专业分包商-类别编码
            String conName = "GAT专业分包商-类别编码" + GTime.getCurrentTime(GTime.FORMAT_14);
            grid3.ui_C_GRID3_FILL_CATEGORY(grid3.table_Row_WebElement_Text.size(), 2, pData.pageREQ.mapContentId.get("类别编码"), conName, "");
            pData.pageREQ.mapDynamicData.replace("专业分包商类别-类别编码", conName);
            GScene.DYNAMIC_DATA.put("专业分包商类别-类别编码", conName);
            GWCtrlFrame.ui_C_SWITCN_DEFAULT();
            //点击一下保存
            ui_C_BTN_TOPMENU("保存", "");
            GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(1));
            //填入GAT专业分包商-类别名称
            conName = "GAT专业分包商-类别名称" + GTime.getCurrentTime(GTime.FORMAT_14);
            ...
            pData.pageREQ.mapDynamicData.replace("专业分包商类别-类别名称", conName);
            GScene.DYNAMIC_DATA.put("专业分包商类别-类别名称", conName);
            GWCtrlFrame.ui_C_SWITCN_DEFAULT();
			pData.bRES = true;
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("专业分包商类别-分包商编码"));
					break;
				}
				default:{
					break;
