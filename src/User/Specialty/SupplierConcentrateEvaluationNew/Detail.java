...
public class Detail {
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("专业分包商集中评价-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("专业分包商集中评价-单据编号", pData.pageREQ.mapDynamicData.get("专业分包商集中评价-单据编号"));
