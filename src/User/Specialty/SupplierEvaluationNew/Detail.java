package User.Specialty.SupplierEvaluationNew;
...
import AutoTestWeb.GWCtrlCheckBox;
...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
            ...
            //填入要评价的【分包商编码】
            GWCtrlPage.ui_C_SELECT_INPUT_BTN(pData.pageREQ.mapContentId.get("企业名称"), "");
            //游标切换至二级窗体
            GWCtrlWindow.windowHandles();
            try {
              
              //取消本机构使用分供商复选框
              GWCtrlCheckBox.ById2Cancel("selfUse");
              
              //专业分包商类别
              GWCtrlGrid3 treeViewGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("参考树"));
              treeViewGrid3.ui_C_SELECT_TREEVIEW(pData.pageREQ.mapDynamicData.get("专业分包商类别-类别编码"));
              //分包商编码
              GWCtrlSelect.ByValue(GWCtrlWebElementId.CN_ID.get("查询方案"), pData.pageREQ.mapDynamicData.get("专业分包商名录-企业编码"));
              GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
              sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("专业分包商名录-企业编码"));
              //确认
              GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 确定"), "");
              
              pData.bRES = true;
              pData.strStatus = "编辑中";
              GLog.logRecordTime(9, "[功能]----选择分包商编码成功\n" + JSON.toJSONString(pData.indexRows));
              }catch (Exception e){
              GWCtrlException.SwtichTo(e, 1, 9, "无可被参考的分包商编码", true);
			...
			GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
        	//填入【评价状态】
        	GWCtrlDropDown.ByValue(pData.pageREQ.mapContentId.get("评价状态"), "试用");
			GWCtrlFrame.ui_C_SWITCN_DEFAULT();
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("专业分包商评价-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("专业分包商评价-单据编号", pData.pageREQ.mapDynamicData.get("专业分包商评价-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("专业分包商评价-单据编号"));
        	pData.bRES = true;
        	AutoTest.GParam.strTestResultCode = "0000";
            AutoTest.GParam.strTestResultMsg = "OK";
		}catch (Exception e){
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("专业分包商评价-单据编号"));
					break;
				}
				default:{
					break;
