...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			//填入【结算单位】
			//点击按钮显示供应商列表二级窗体
			GWCtrlPage.ui_C_SELECT_INPUT_BTN(pData.pageREQ.mapContentId.get("结算单位"), "");
			//游标切换至二级窗体
			GWCtrlWindow.windowHandles();
			//搜索并选中
			GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
			sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("运杂费结算单-结算单位"));
			pData.gvGrid3 = null;
	...
	private void ui_C_SELECT_BILL() {
		...
		try {
			...
			try {
				//选择
				GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
				sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("运杂费记录单-单据编号"));
				pData.gvGrid3 = null;
				//确认移入
				...
				pData.indexRows++;
	        	pData.indexRowsMaterial.put("费用明细", pData.indexRows);
				GLog.logRecordTime(9, JSON.toJSONString(pData.indexRows));	

				pData.bRES = true;
				pData.strStatus = "编辑中";
				GLog.logRecordTime(9, "[功能]----选择运杂费记录单成功");
				exist = true;
			}catch (Exception e){
			...
			if(exist) {
				//切换页签
				pData.pageREQ.MODULE_WAIT_ID = GWCtrlWebElementId.CN_ID.get("运杂费结算单-费用明细");
				ui_C_CLICK_TAB("费用明细");
				ui_C_INPUT_STOCK();
			}
		}catch (Exception e){
	...
	public void ui_C_M_DETAILS() {
		pData.pageREQ.MODULE_WAIT_ID = GWCtrlWebElementId.CN_ID.get("其他费用");
		ui_C_CLICK_TAB("其他费用");
		ui_C_NEW_COST_ITEM();
		ui_C_ASSERT();
		ui_C_BTN_TOPMENU("保存", "");
	 ...
	 */
	@Step("新增费用项")
	private void ui_C_NEW_COST_ITEM() {
		GLog.logRecordTime(9, "[功能]----新增费用项");
		try {
			GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("新增费用项"), "");

			//添加费用项
			boolean exist = false;
			try {
				pData.indexRows++;
				pData.indexRowsMaterial.put("新增费用项", pData.indexRows);
				
				pData.bRES = true;
				pData.strStatus = "编辑中";
				GLog.logRecordTime(9, "[功能]----新增费用项成功\n" + JSON.toJSONString(pData.indexRows));
				exist = true;
			}catch (Exception e){
				GWCtrlException.SwtichTo(e, 1, 9, "新增费用项失败", true);
			}
			...
			pData.bRES = false;
			GWCtrlException.SwtichTo(e, 1, 9, "[功能]----新增费用项失败", true);
		}
		GWCtrlPage.ui_D_IFRAME_INDEX(2, "id", pData.pageREQ.MODULE_WAIT_ID);
		GTestCase.setTestCaseRst(pData.bRES);
	}
	...
	private void ui_C_INPUT_DICT() {
		GLog.logRecordTime(9, "[功能]----填写其他费用必填项");
		try {
			GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
			
			//根据行列坐标确认填写材料明细的位置
			//填写【费用项名称】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("其他费用"));
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					4, 
					pData.pageREQ.mapContentId.get("费用项名称"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("运杂费结算单-其他费用-费用项名称")), 
					GWCtrlWebElementId.CN_ID.get("其他费用"));
			//填写【含税单价】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("其他费用"));
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					6, 
					pData.pageREQ.mapContentId.get("含税单价"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("运杂费结算单-其他费用-含税单价")), 
					GWCtrlWebElementId.CN_ID.get("其他费用"));
			//填写【数量】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("其他费用"));
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("运杂费结算单-其他费用-数量")), 
					GWCtrlWebElementId.CN_ID.get("其他费用"));
			//填写【税率】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("其他费用"));
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("运杂费结算单-其他费用-税率(%)")), 
					GWCtrlWebElementId.CN_ID.get("其他费用"));
			
			//点击空白处确认编辑结果
			GWCtrlDivClick.ById(GWCtrlWebElementId.CN_ID.get("其他费用"));
			...
			pData.bRES = true;
			pData.strStatus = "编辑中";
			GLog.logRecordTime(9, "[功能]----填写其他费用必填项成功");
		}catch (Exception e){
			pData.bRES = false;
			GWCtrlException.SwtichTo(e, 1, 9, "[功能]----填写其他费用必填项失败", true);
		}
		GWCtrlPage.ui_D_IFRAME_INDEX(2, "id", pData.pageREQ.MODULE_WAIT_ID);
		GTestCase.setTestCaseRst(pData.bRES);
	}
	...
	private void ui_C_INPUT_STOCK() {
		GLog.logRecordTime(9, "[功能]----填写费用明细");
		
		try {
			GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
			
			//填写【结算单价】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("运杂费结算单-费用明细"));
			pData.gvGrid3.showWebElementTextTableMap();
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					8, 
					pData.pageREQ.mapContentId.get("费用明细-含税结算金额"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("运杂费结算单-费用明细-含税结算金额")), 
					GWCtrlWebElementId.CN_ID.get("运杂费结算单-费用明细"));
			//点击空白处确认编辑结果
			GWCtrlDivClick.ById(GWCtrlWebElementId.CN_ID.get("运杂费结算单-费用明细"));
			//释放内存，在【参数校验】步骤中会重新读取细表
			pData.gvGrid3 = null;
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("运杂费结算单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("运杂费结算单-单据编号", pData.pageREQ.mapDynamicData.get("运杂费结算单-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("运杂费结算单-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			//页面填入的含税金额
			double ac = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(1, 8).replaceAll(",", "")));
			//页面填入的税率
			double ff = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(1, 9).replaceAll(",", "")));
			...
			//页面实际显示的无税金额
			String TestCompare = (pData.gvGrid3.ui_C_GRID3_VALUE(1, 11).replaceAll(",", ""));
			...
			if(GAssert.assertStringEqual(TestResult, TestCompare)) {
				...
				AutoTest.GLog.logRecord(9, "运杂费结算单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 18, AutoTest.GParam.GREEN_PASS);
			}else {
				pData.bRES = false;
				AutoTest.GParam.strTestResultCode = "9999";
				AutoTest.GParam.strTestResultMsg = "ERROR";
				AutoTest.GParam.gRes = "[算法]含税金额=无税金额*(1+税率)\n则无税金额=含税金额/(1+税率)" + "\n[预期无税金额]:" + TestResult  + "\n[实际无税金额]:" + TestCompare+ "\n[算法验证]----未通过";
				AutoTest.GLog.logRecord(9, "运杂费结算单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 18, Color.RED);
			}
	 ...
	 */
	@Step("算法校验-费用项明细")
	private void ui_C_ASSERT_FYXMX(){
		GLog.logRecordTime(9, "[算法校验]");
		try {
			GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("其他费用"));
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("运杂费结算单-单据编号"));
					break;
				}
				default:{
					break;
