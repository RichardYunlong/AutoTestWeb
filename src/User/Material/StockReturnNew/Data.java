...
public class Data {
	...
	public Data() {
		...
		//构造一个临时驱动
		pageREQ = new GWCtrlMainPage("材料退库单");
		//顶层菜单
		...
		//单据菜单
		pageREQ.mapTopTabId.put("选择已出库材料", "btnSelectYCKCL");
		//基本信息
		pageREQ.mapContentId.put("单据编号", "Efd_Code");
		pageREQ.mapContentId.put("退库日期", "Efd_BizDate");
		pageREQ.mapContentId.put("库房", "Efd_KFZD_Name");
		pageREQ.mapContentId.put("材料类别", "Efd_CLLB_Name");
		pageREQ.mapContentId.put("退料单位", "Efd_TLDW_Name");
		pageREQ.mapContentId.put("退料人", "Efd_TLR");
		pageREQ.mapContentId.put("备注", "Efd_evRemark_Remark");
		//材料明细
		pageREQ.mapContentId.put("退库单价", "gvCLMX_col_DJ_editor");
		pageREQ.mapContentId.put("退库数量", "gvCLMX_col_SL_editor");
		//等待标签
		pageREQ.MODULE_WAIT_ID = GWCtrlWebElementId.CN_ID.get("材料明细");
		//日期
		strDate = "2020-04-09";
		//动态数据
		pageREQ.mapDynamicData.put("材料退库单-供应商", GScene.DYNAMIC_DATA.get("材料退库单-供应商"));
		pageREQ.mapDynamicData.put("材料退库单-退料单位", GScene.DYNAMIC_DATA.get("材料退库单-退料单位"));
		pageREQ.mapDynamicData.put("材料退库单-材料编码", GScene.DYNAMIC_DATA.get("材料退库单-材料编码"));
		pageREQ.mapDynamicData.put("材料退库单-材料明细-退库单价", GScene.DYNAMIC_DATA.get("材料退库单-材料明细-退库单价"));
		pageREQ.mapDynamicData.put("材料退库单-材料明细-退库数量", GScene.DYNAMIC_DATA.get("材料退库单-材料明细-退库数量"));
		pageREQ.mapDynamicData.put("材料出库单-单据编号", GScene.DYNAMIC_DATA.get("材料出库单-单据编号"));
		pageREQ.mapDynamicData.put("材料退库单-单据编号", "");
	}
}
