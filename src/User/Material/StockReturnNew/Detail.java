package User.Material.StockReturnNew;
...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
        	...
        	pData.bRES = true;
			pData.strStatus = "编辑中";
			GLog.logRecordTime(9, "[功能]----必填项填写成功");
		}catch (Exception e){
			pData.bRES = false;
			GLog.logRecordTime(9, "[功能]----必填项填写失败");
	...
	private void ui_C_SELECT_DICT() {
		GLog.logRecordTime(9, "[功能]----选择已出库材料");
		try {
			GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("选择已出库材料"), "");
			GWCtrlWindow.windowHandles();
			...
			try {
				...
				if(pData.billCode != null && !pData.billCode.equals("")) {
					...
					billCode = pData.pageREQ.mapDynamicData.get("材料出库单-单据编号");
				}
				...
				pData.bRES = true;
				pData.strStatus = "编辑中";
				GLog.logRecordTime(9, "[功能]----选择已出库材料成功\n" + JSON.toJSONString(pData.indexRows));
				exist = true;
			}catch (Exception e){
				GLog.logRecordTime(9, "无可被参考的已出库材料");
			...
			pData.bRES = false;
			GLog.logRecordTime(9, "[功能]----选择已出库材料失败");
			if (AutoTest.GTestCase.dTSSTYLE == 1)
				AutoTest.GParam.gRes = GException.getExceptionAllinformation(e);
			e.printStackTrace();
		}
	...
	private void ui_C_INPUT_DICT() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					6, 
					pData.pageREQ.mapContentId.get("退库单价"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料退库单-材料明细-退库单价")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//填写【数量】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("材料明细"));
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					7, 
					pData.pageREQ.mapContentId.get("退库数量"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料退库单-材料明细-退库数量")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//点击空白处确认编辑结果
	 ...
	 */
	@Step("参数提取")
	private void ui_PARAMS() {
		GLog.logRecordTime(9, "[提取公共参数]");
		try {
        	//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("材料退库单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("材料退库单-单据编号", pData.pageREQ.mapDynamicData.get("材料退库单-单据编号"));

			GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("材料退库单-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			//退库数量
			double ac = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 7).replaceAll(",", "")));
			//金额计算值
			double rc = dc * ac;
			DecimalFormat dR = new DecimalFormat("#.00");
			String TestResult = dR.format(rc);//理论计算的无税金额保留小数点后两位
			//金额显示值
			String TestCompare = (pData.gvGrid3.ui_C_GRID3_VALUE(2, 8).replaceAll(",", ""));
			...
			if(GAssert.assertStringEqual(TestResult, TestCompare)) {
				...
				AutoTest.GParam.gRes = "[算法]金额=退库单价*退库数量" + 
						"\n[金额]:" + TestCompare + "\n[算法验证]----通过";
				AutoTest.GLog.logRecord(9, "材料退库单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, AutoTest.GParam.GREEN_PASS);
			}else {
				pData.bRES = false;
				...
				AutoTest.GParam.gRes = "[算法]金额=退库单价*退库数量" + 
						"\n[金额]:" + TestCompare + "\n[算法验证]----未通过";
				AutoTest.GLog.logRecord(9, "材料退库单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, Color.RED);
			}
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("材料退库单-单据编号"));
					break;
				}
				default:{
					break;
