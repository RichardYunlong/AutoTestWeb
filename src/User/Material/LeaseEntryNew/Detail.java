...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			//填入【进场日期】
        	GWCtrlPage.ui_C_SELECT_INPUT_BTN(pData.pageREQ.mapContentId.get("进场日期"), "");
        	GWCtrlPage.ui_C_SELECT_DATE_STR("今天");
	...
	public void ui_C_M_DETAILS() {
		GLog.logRecord(8, "租赁周转材料进场单", "res", "填写材料明细", 5000, 0, 0, "", "微软雅黑", 20, Color.RED);
		//设置等待
		pData.pageREQ.MODULE_WAIT_ID = GWCtrlWebElementId.CN_ID.get("材料明细");
		ui_C_CLICK_TAB("材料明细");
		//ui_C_SELECT_DICT();
		GWCtrlLog.TakesScreenshot("_2.png");
		ui_C_CLICK_REFER_TO("选择合同材料");
		ui_C_ASSERT();
		ui_C_BTN_TOPMENU("保存", "");
		GWCtrlAllure.makeScreenShot("租赁周转材料进场单-详情区-材料明细-功能截图");
	}
	...
	private void ui_C_CLICK_REFER_TO(String strModuelName) {
		...
		try {
			GLog.logRecord(9, "租赁周转材料进场单", "res", 
					"参考之前创建的周转材料租赁合同[" + pData.pageREQ.mapDynamicData.get("周转材料租赁合同-单据编号") + "]", 
			...
			try {
				//材料
				GWCtrlSelect.ByValue(GWCtrlWebElementId.CN_ID.get("查询方案"), pData.pageREQ.mapDynamicData.get("租赁周转材料进场单-材料编码"));
				GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
				sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("租赁周转材料进场单-材料编码"));
				//移入
				GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 移入"), "");
				//确认
				GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 确定"), "");
	...
	private void ui_C_INPUT_REFER() {
		...
		try {
			...
			//填写【天数】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("材料明细"));
			pData.gvGrid3.ui_C_GRID3_BROWSE(2, 5, pData.pageREQ.mapContentId.get("使用单位"));
			//游标切换至二级窗体
			GWCtrlWindow.windowHandles();
			//搜索并选中
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("内部单位"), "");
			GWCtrlPage.ui_C_SEARCH_COO(pData.pageREQ.mapDynamicData.get("租赁周转材料进场单-使用单位"));
			GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 确定"), "");
			GWCtrlTime.Pause(3);
        	//游标切换回主窗体
        	GWCtrlWindow.windowHandlePre();
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("租赁周转材料进场单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("租赁周转材料进场单-单据编号", pData.pageREQ.mapDynamicData.get("租赁周转材料进场单-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("租赁周转材料进场单-单据编号"));
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			String TestResult = GScene.DYNAMIC_DATA.get("周转材料租赁合同-数量");
			//进场数量
