...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			pData.bRES = true;
			pData.strStatus = "编辑中";
			GLog.logRecordTime(9, "[功能]----基本信息填写完成");
		}catch (Exception e){
			pData.bRES = false;
...
//		ui_C_CLICK_REFER_TO("参考需用计划");
	...
	private void ui_C_SELECT_DICT() {
		...
		try {
			GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("选择字典材料"), "");
			GWCtrlWindow.windowHandles();
			...
			try {
				//等待分页查询结果加载完成
				GWCtrlWait.Wait2BeClickableByCssSelector(GTestIndicators.PageShowTime, GText.getCssSelectorTxt("div", "class", "x-grid3-cell-inner x-grid3-col-col_Code x-unselectable"));
				//选择
				GWCtrlPage.ui_C_WAIT_CLICK("", pData.pageREQ.mapContentXpath.get("第一个材料"));
				//确认移入
				GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 移入"), "");
				//确认选定
				GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 确定"), "");
			...
			pData.bRES = false;
			GLog.logRecordTime(9, "[功能]----选择字典材料失败");
			if (AutoTest.GTestCase.dTSSTYLE == 1)
				AutoTest.GParam.gRes = GException.getExceptionAllinformation(e);
			e.printStackTrace();
		}
		GWCtrlPage.ui_D_IFRAME_INDEX(2, "id", pData.pageREQ.MODULE_WAIT_ID);//操作完再切回材料明细
	}
	...
	private void ui_C_INPUT_DICT() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料需要计划-材料明细-本期计划数量")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//点击空白处确认编辑结果
			GWCtrlDivClick.ById(pData.pageREQ.mapContentId.get("空白处"));
			GWCtrlFrame.ui_C_SWITCN_DEFAULT();
			
			pData.bRES = true;
			pData.strStatus = "编辑中";
			System.out.println("[功能点]----字典材料必填项填写成功");
		}catch (Exception e){
			pData.bRES = false;
			System.out.println("[功能点]----字典材料必填项填写失败");
			e.printStackTrace();
		}
		GWCtrlPage.ui_D_IFRAME_INDEX(2, "id", pData.pageREQ.MODULE_WAIT_ID);
	}
	...
	private void ui_C_ASSERT(){
		...
		try {
			if(GScene.gSceneName.equals("GDemandPlanQuantityOccupancy")) {
				...
				//更新材料占位数量
				GScene.materialCountPlus("材料采购合同-材料明细-占位数量", pData.pageREQ.mapDynamicData.get("材料采购合同-材料明细-数量"));
				//占位验证
			...
			switch(menuName){
				...
				case "删除":{
					...
					if(GWCtrlTopMenuBtn.ui_C_GET_TOPMENUBTN_TAB2_STATUS(menuName, GTestIndicators.PageShowTime, pData.pageREQ.MODULE_WAIT_ID)) {
						if(GWCtrlTopMenuBtn.ui_C_GET_TOPMENUBTN_TAB2_STATUS("取消提交", GTestIndicators.PageShowTime, pData.pageREQ.MODULE_WAIT_ID)) {
							if(GWCtrlTopMenuBtn.ui_C_GET_TOPMENUBTN_TAB2_STATUS(menuName, GTestIndicators.PageShowTime, pData.pageREQ.MODULE_WAIT_ID)) {
								pData.strStatus = "编辑中";	
							}
						}
