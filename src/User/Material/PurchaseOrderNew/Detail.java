...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("材料采购订单-供应商"));
			pData.gvGrid3 = null;
	...
	private void ui_C_SELECT_DICT() {
		...
		try {
			...
			try {
				...
				treeViewGrid3.ui_C_SELECT_TREEVIEW(pData.pageREQ.mapDynamicData.get("材料采购比价单-材料类型编码"));
				//材料
				GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
				sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("材料采购比价单-材料编码"));
				//移入
	...
	private void ui_C_CLICK_REFER_TO(String strModuelName) {
		...
		try {
			...
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("参考需用计划"), "");
			GWCtrlWindow.windowHandles();
	...
	private void ui_C_INPUT_DICT() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料采购订单-材料明细-含税单价")), 
			...
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料采购订单-材料明细-数量")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//填写【税率】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("材料明细"));
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料采购订单-材料明细-税率(%)")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			...
			pData.bRES = false;
			GWCtrlException.SwtichTo(e, 1, 9, "[功能]----填写字典材料必填项填写失败", true);
		}
		GWCtrlPage.ui_D_IFRAME_INDEX(2, "id", pData.pageREQ.MODULE_WAIT_ID);
		GTestCase.setTestCaseRst(pData.bRES);
	}
	...
	private void ui_C_INPUT_REFER() {
		...
		try {
			...
			GLog.logRecord(9, "材料采购订单", "res", 
					"含税单价:" + String.valueOf(pData.pageREQ.mapDynamicData.get("材料采购订单-材料明细-含税单价")) 
					+ "\n数量:" + String.valueOf(pData.pageREQ.mapDynamicData.get("材料采购订单-材料明细-数量")) 
					+ "\n税率:" + String.valueOf(pData.pageREQ.mapDynamicData.get("材料采购订单-材料明细-税率(%)")), 
			...
			pData.bRES = false;
			GWCtrlException.SwtichTo(e, 1, 9, "[功能]----填写参考必填项填写失败", true);
		}
		GWCtrlPage.ui_D_IFRAME_INDEX(2, "id", pData.pageREQ.MODULE_WAIT_ID);
		GTestCase.setTestCaseRst(pData.bRES);
	}
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("材料采购订单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("材料采购订单-单据编号", pData.pageREQ.mapDynamicData.get("材料采购订单-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("材料采购订单-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			//页面填入的含税金额
			double ac = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(1, 11).replaceAll(",", "")));
			//页面填入的税率
			double ff = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(1, 13).replaceAll(",", "")));
			...
			//页面实际显示的无税金额
			String TestCompare = (pData.gvGrid3.ui_C_GRID3_VALUE(1, 15).replaceAll(",", ""));
			...
			if(GAssert.assertStringEqual(TestResult, TestCompare)) {
				...
				AutoTest.GLog.logRecord(9, "材料采购订单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 18, AutoTest.GParam.GREEN_PASS);
			}else {
				pData.bRES = false;
				AutoTest.GParam.strTestResultCode = "9999";
				AutoTest.GParam.strTestResultMsg = "ERROR";
				AutoTest.GParam.gRes = "[算法]含税金额=无税金额*(1+税率)\n则无税金额=含税金额/(1+税率)" + "\n[预期无税金额]:" + TestResult  + "\n[实际无税金额]:" + TestCompare+ "\n[算法验证]----未通过";
				AutoTest.GLog.logRecord(9, "材料采购订单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 18, Color.RED);
			}
	...
	private void ui_C_BTN_TOPMENU(String menuName, String strBillCode) {
		...
		try {
			...
			pData.bRES = false;
			GWCtrlException.SwtichTo(e, 1, 9, "[功能]----对目标[" + strBillCode + "]执行[" + menuName + "]失败", true);
		}
		GTestCase.setTestCaseRst(pData.bRES);
	}
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("材料采购订单-单据编号"));
					break;
				}
				default:{
					break;
