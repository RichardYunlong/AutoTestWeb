...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			//填入【收料单位】
			GWCtrlPage.ui_C_SELECT_INPUT_BTN("Efd_SLDW_Name", "");
			GWCtrlWindow.windowHandles();
			//搜索并选中
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("内部单位"), "");
			GWCtrlPage.ui_C_SEARCH_COO(pData.pageREQ.mapDynamicData.get("材料库房调拨单-收料单位"));
	...
	private void ui_C_INPUT_STOCK() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					5, 
					pData.pageREQ.mapContentId.get("调拨单价"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料库房调拨单-在库材料-调拨单价")), 
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料库房调拨单-在库材料-调拨数量")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//点击空白处确认编辑结果
	...
	private void ui_C_PARAMS() {
		...
		try {
        	//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("材料库房调拨单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("材料库房调拨单-单据编号", pData.pageREQ.mapDynamicData.get("材料库房调拨单-单据编号"));


			GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("材料库房调拨单-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			//库存单价
			double cc = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 3).replaceAll(",", "")));
			//调拨单价
			...
			if(GAssert.assertStringEqual(TestResult, TestCompare)) {
				...
				AutoTest.GParam.gRes = "[算法]金额差=(调拨单价-库存单价)*调拨数量" + 
						"\n[金额差]:" + TestCompare + "\n[算法验证]----通过";
				AutoTest.GLog.logRecord(9, "材料库房调拨单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, AutoTest.GParam.GREEN_PASS);
			}else {
				pData.bRES = false;
				...
				AutoTest.GParam.gRes = "[算法]金额差=(调拨单价-库存单价)*调拨数量" + 
						"\n[金额差]:" + TestCompare + "\n[算法验证]----未通过";
				AutoTest.GLog.logRecord(9, "材料库房调拨单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, Color.RED);
			}
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("材料库房调拨单-单据编号"));
					break;
				}
				default:{
					break;
