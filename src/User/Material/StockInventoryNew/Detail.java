	...
	private void ui_C_SELECT_STOCK() {
		...
		try {
			...
			try {
				...
				sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("材料库房盘点单-材料编码"));
				//确认移入
			...
			pData.bRES = false;
			GLog.logRecordTime(9, "[功能]----参考在库材料失败");
			if (AutoTest.GTestCase.dTSSTYLE == 1)
				AutoTest.GParam.gRes = GException.getExceptionAllinformation(e);
			e.printStackTrace();
		}
	...
	private void ui_C_INPUT_STOCK() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					6, 
					pData.pageREQ.mapContentId.get("实际单价"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料库房盘点单-在库材料-实际单价")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));

			//填写【数量】 此处对数量进行算法校验，暂不主动输入
//			GWCtrlInputFill.ByXpathFromClickDiv(Data.pageREQ.mapContentXpath.get("实际数量")
//					, Data.pageREQ.mapContentId.get("实际数量")
//					, String.valueOf(pData.pageREQ.mapDynamicData.get("材料库房盘点单-在库材料-实际数量"))
//					, Data.pageREQ.mapContentId.get("空白处"));
			//点击空白处确认编辑结果
	...
	private void ui_C_PARAMS() {
		...
		try {
        	//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("材料库房盘点单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("材料库房盘点单-单据编号", pData.pageREQ.mapDynamicData.get("材料库房盘点单-单据编号"));

			GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("材料库房盘点单-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			GLog.logRecordTime(9, "[算法验证]");
			GLog.logRecordTime(9, "[入库]：+" + GScene.DYNAMIC_DATA.get("材料入库单-材料明细-数量"));
			GLog.logRecordTime(9, "[退货]：-" + GScene.DYNAMIC_DATA.get("材料退货单-材料明细-数量"));
			GLog.logRecordTime(9, "[出库]：-" + GScene.DYNAMIC_DATA.get("材料出库单-材料明细-数量"));
			GLog.logRecordTime(9, "[退库]：+" + GScene.DYNAMIC_DATA.get("材料退库单-材料明细-退库数量"));
			GLog.logRecordTime(9, "[调拨]：-" + GScene.DYNAMIC_DATA.get("材料库房调拨单-在库材料-调拨数量"));
			GLog.logRecordTime(9, "[报损]：-" + GScene.DYNAMIC_DATA.get("材料库房报损单-在库材料-报损数量"));
			Integer ac = Integer.valueOf(GScene.DYNAMIC_DATA.get("材料入库单-材料明细-数量"))
				   - Integer.valueOf(GScene.DYNAMIC_DATA.get("材料退货单-材料明细-数量"))
				   - Integer.valueOf(GScene.DYNAMIC_DATA.get("材料出库单-材料明细-数量"))
				   + Integer.valueOf(GScene.DYNAMIC_DATA.get("材料退库单-材料明细-退库数量"))
				   - Integer.valueOf(GScene.DYNAMIC_DATA.get("材料库房调拨单-在库材料-调拨数量"))
				   - Integer.valueOf(GScene.DYNAMIC_DATA.get("材料库房报损单-在库材料-报损数量"));
			
			Integer rc = 0;
			try {
				rc = Integer.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 7).replaceAll(",", "")));
			}catch (Exception e){
				pData.bRES = false;
				GLog.logRecordTime(9, "账面数量=实际数量\\n相同盘点时间段内，只能有一张盘点单,否则将获取不到在库材料数据");
				e.printStackTrace();
				rc = 0;
			}
			
			if(GAssert.assertIntegerEqual(ac, rc)) {
				pData.bRES = true;
				AutoTest.GParam.strTestResultCode = "0000";
				AutoTest.GParam.strTestResultMsg = "OK";
				AutoTest.GParam.gRes = "[算法]账面数量=实际数量\n相同盘点时间段内，只能有一张盘点单" + 
							"\n[账面数量]:" + String.valueOf(ac) + "\n[实际数量]:" + String.valueOf(rc) + "\n[算法验证]----通过";
				AutoTest.GLog.logRecord(9, "材料库房盘点单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, AutoTest.GParam.GREEN_PASS);
			}else {
				pData.bRES = false;
				...
				AutoTest.GParam.gRes = "[算法]账面数量=实际数量\n相同盘点时间段内，只能有一张盘点单" + 
							"\n[账面数量]:" + String.valueOf(ac) + "\n[实际数量]:" + String.valueOf(rc) + "\n[算法验证]----未通过";
				AutoTest.GLog.logRecord(9, "材料库房盘点单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, Color.RED);
			}
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("材料库房盘点单-单据编号"));
					break;
				}
				default:{
					break;
