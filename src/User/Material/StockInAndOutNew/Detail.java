...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("材料直入直出单-供应商"));
			pData.gvGrid3 = null;
			...
			//填入【领料单位】
			//点击按钮显示甲方列表二级窗体
        	GWCtrlPage.ui_C_SELECT_INPUT_BTN("Efd_LLDW_Name", "");
			//游标切换至二级窗体
			GWCtrlWindow.windowHandles();
			//搜索并选中
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("内部单位"), "");
			GWCtrlPage.ui_C_SEARCH_COO(pData.pageREQ.mapDynamicData.get("材料直入直出单-领料单位"));
			GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 确定"), "");
        	//游标切换回主窗体
        	GWCtrlWindow.windowHandlePre();
        	GWCtrlFrame.ui_C_SWITCN_DEFAULT();
	...
	private void ui_C_CLICK_REFER_TO(String strModuelName) {
		...
		try {
			...
			try {
				...
				pData.indexRows++;
				pData.indexRowsMaterial.put("参考需用计划", pData.indexRows);
	...
	private void ui_C_INPUT_REFER() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料直入直出单-材料明细-含税单价")), 
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料直入直出单-材料明细-数量")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//填写【税率】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("材料明细"));
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料直入直出单-材料明细-税率(%)")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//点击空白处确认编辑结果
	...
	private void ui_C_PARAMS() {
		...
		try {
        	//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("材料直入直出单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("材料直入直出单-单据编号", pData.pageREQ.mapDynamicData.get("材料直入直出单-单据编号"));
        	
			GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("材料直入直出单-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			//含税单价
			double dc = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 3).replaceAll(",", "")));
			//数量
			double ac = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 4).replaceAll(",", "")));
			//熟虑
			double ff = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 10).replaceAll(",", "")));
			//计成本金额计算值
			double rc = dc / ((double)1.00 + ff/100) * ac;
			...
			if(GAssert.assertStringEqual(TestResult, TestCompare)) {
				...
				AutoTest.GParam.gRes = "[算法]计成本金额=含税单价/(1-税率)*100%*数量" + 
						"\n[计成本金额]:" + TestCompare + "\n[算法验证]----通过";
				AutoTest.GLog.logRecord(9, "材料调拨结算单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, AutoTest.GParam.GREEN_PASS);
			}else {
				pData.bRES = false;
				...
				AutoTest.GParam.gRes = "[算法]计成本金额=含税单价/(1-税率)*100%*数量" + 
						"\n[计成本金额]:" + TestCompare + "\n[算法验证]----未通过";
				AutoTest.GLog.logRecord(9, "材料调拨结算单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, Color.RED);
			}
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("材料直入直出单-单据编号"));
					break;
				}
				default:{
					break;
