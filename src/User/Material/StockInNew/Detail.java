package User.Material.StockInNew;
...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			GWCtrlWindow.windowHandles();
	         //判断当前供应商单位
            switch (GScene.DYNAMIC_DATA.get("供应商单位")) {
              case "内部单位":{
                //搜索并选中
                GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("内部单位"), "");
                GWCtrlPage.ui_C_SEARCH_COO(pData.pageREQ.mapDynamicData.get("材料入库单-供应商"));
              ...
              case "外部单位":{
                ...
                sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("材料入库单-供应商"));
                pData.gvGrid3 = null;
                GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 确定"), "");
                break;
              }
            ...
            GWCtrlDropDown.ByValue(pData.pageREQ.mapContentId.get("供应类型"), GScene.DYNAMIC_DATA.get("供应类型"));
            GWCtrlFrame.ui_C_SWITCN_DEFAULT();
	...
	public void ui_C_M_DETAILS() {
		...
		//通过“参考”添加材料并填写必填项
        ui_C_CLICK_REFER_TO(GScene.DYNAMIC_DATA.get("参考"));
	...
	private void ui_C_CLICK_REFER_TO(String strModuelName) {
		...
		try {
			AutoTest.GLog.logRecord(9, "材料入库单", "res", "引用"+strModuelName, 8000, 0, 0, "", "微软雅黑", 20, Color.RED);
            ...
            try {
              ...
              switch (strModuelName) {
                case "参考需用计划":
                  //类别
                  treeViewGrid3.ui_C_SELECT_TREEVIEW(pData.pageREQ.mapDynamicData.get("材料需用计划-单据编号"));
                  //材料
                  sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
                  sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("材料需用计划-单据编号"));
                  break;
                case "参考其他单位调入":
                  //类别
                  treeViewGrid3.ui_C_SELECT_TREEVIEW(pData.pageREQ.mapDynamicData.get("参考单据编号"));
                  //材料
                  sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
                  sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("参考单据编号"));
                  break;
              }
              //移入
              ...
              GWCtrlException.SwtichTo(e, 1, 9, "二级窗体 取消", true);
          }
	...
	private void ui_C_INPUT_REFER() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料入库单-材料明细-含税单价")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//填写【数量】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("材料明细"));
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					4, 
					pData.pageREQ.mapContentId.get("入库数量"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料入库单-材料明细-数量")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//填写【税率】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("材料明细"));
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料入库单-材料明细-税率(%)")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//点击空白处确认编辑结果
	...
	private void ui_C_PARAMS() {
		...
		try {
        	//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("材料入库单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("材料入库单-单据编号", pData.pageREQ.mapDynamicData.get("材料入库单-单据编号"));

			GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("材料入库单-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			//含税金额
			double dc = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 5).replaceAll(",", "")));
			//税额
			double ac = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 12).replaceAll(",", "")));
			//无税金额计算值
			double rc = dc - ac;
			DecimalFormat dR = new DecimalFormat("#.00");
			String TestResult = dR.format(rc);//理论计算的无税金额保留小数点后两位
			//页面实际显示的记成本金额
			String TestCompare = (pData.gvGrid3.ui_C_GRID3_VALUE(2, 15).replaceAll(",", ""));
			...
			if(GAssert.assertStringEqual(TestResult, TestCompare)) {
				...
				AutoTest.GParam.gRes = "[算法]成本金额=含税金额-税额" + 
						"\n[成本金额]:" + TestCompare + "\n[含税金额]:" + String.valueOf(dc) + "\n[税额]:" + String.valueOf(ac) + "\n[算法验证]----通过";
				AutoTest.GLog.logRecord(9, "材料入库单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, AutoTest.GParam.GREEN_PASS);
			}else {
				pData.bRES = false;
				...
				AutoTest.GParam.gRes = "[算法]成本金额=含税金额-税额" + 
						"\n[成本金额]:" + TestCompare + "\n[含税金额]:" + String.valueOf(dc) + "\n[税额]:" + String.valueOf(ac) + "\n[算法验证]----未通过";
				AutoTest.GLog.logRecord(9, "材料入库单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, Color.RED);
			}
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("材料入库单-单据编号"));
					break;
				}
				default:{
					break;
