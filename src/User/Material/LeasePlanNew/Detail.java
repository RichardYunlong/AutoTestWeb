...
public class Detail {
	...
	private void ui_C_INPUT_REFER() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					5, 
					pData.pageREQ.mapContentId.get("计费单价"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("周转材料租赁计划-计费单价")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//填写【数量】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("材料明细"));
			pData.gvGrid3.showWebElementTextTableMap();
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					6, 
					pData.pageREQ.mapContentId.get("计划租赁数量"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("周转材料租赁计划-本期计划量")), 
					GWCtrlWebElementId.CN_ID.get("材料明细"));
			//点击空白处确认编辑结果
			GWCtrlDivClick.ById(GWCtrlWebElementId.CN_ID.get("材料明细"));
			//释放内存，在【参数校验】步骤中会重新读取细表
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("周转材料租赁计划-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("周转材料租赁计划-单据编号", pData.pageREQ.mapDynamicData.get("周转材料租赁计划-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("周转材料租赁计划-单据编号"));
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			if(GAssert.assertStringEqual(TestResult, TestCompare)) {
				...
				AutoTest.GParam.gRes = "[算法]\n[计划租赁单价]:" + String.valueOf(ac) + "\n[计划租赁数量]:" + String.valueOf(dc) + "\n[金额]:" + TestCompare + "\n[算法验证]----通过";
