...
public class Detail {
	...
	public void ui_C_M_DETAILS() {
		GLog.logRecord(8, "租赁周转材料出场单", "res", "填写材料明细", 5000, 0, 0, "", "微软雅黑", 20, Color.RED);
		//设置等待
		pData.pageREQ.MODULE_WAIT_ID = pData.pageREQ.mapContentId.get("出场明细");
		ui_C_CLICK_TAB("出场明细");
		//ui_C_SELECT_DICT();
		...
		ui_C_ASSERT();
		ui_C_BTN_TOPMENU("保存", "");
		GWCtrlAllure.makeScreenShot("租赁周转材料出场单-详情区-材料明细-功能截图");
	}
	...
	private void ui_C_INPUT_DICT() {
		...
		try {
			...
			//根据行列坐标确认填写材料明细的位置			
			pData.gvGrid3 = new GWCtrlGrid3("id", pData.pageREQ.mapContentId.get("出场明细"));
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
									8, 
									pData.pageREQ.mapContentId.get("本期计划量"), 
									pData.pageREQ.mapDynamicData.get("材料需用计划-本期计划量"), 
									pData.pageREQ.mapContentId.get("出场明细"));
			
			//点击空白处确认编辑结果
			GWCtrlDivClick.ById(pData.pageREQ.mapContentId.get("出场明细"));
	...
	private void ui_C_CLICK_REFER_TO(String strModuelName) {
		...
		try {
			GLog.logRecord(9, "租赁周转材料出场单", "res", 
					"[" + strModuelName + "]", 
			...
			try {
				//材料
				GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
				sourceGrid3.ui_C_SELECT_SOURCE("使用单位", pData.pageREQ.mapDynamicData.get("租赁周转材料出场单-使用单位"));
				//移入
				GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 移入"), "");
				//确认
				GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 确定"), "");
	...
	private void ui_C_INPUT_REFER() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
									6, 
									pData.pageREQ.mapContentId.get("出场数量"), 
									pData.pageREQ.mapDynamicData.get("租赁周转材料出场单-出场数量"), 
									pData.pageREQ.mapContentId.get("出场明细"));
			//点击空白处确认编辑结果
			GWCtrlDivClick.ById(pData.pageREQ.mapContentId.get("出场明细"));
			//释放内存，在【参数校验】步骤中会重新读取细表
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("租赁周转材料出场单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("租赁周转材料出场单-单据编号", pData.pageREQ.mapDynamicData.get("租赁周转材料出场单-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("租赁周转材料出场单-单据编号"));
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			String TestResult = GScene.DYNAMIC_DATA.get("租赁周转材料出场单-出场数量");
			//进场数量
			String TestCompare = (pData.gvGrid3.ui_C_GRID3_VALUE(2, 6).replaceAll(",", ""));
