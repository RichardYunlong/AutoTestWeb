package User.Material.LeaseAllotNew;
...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			//填入【调出单位】
			//点击按钮显示甲方列表二级窗体
			GWCtrlPage.ui_C_SELECT_INPUT_BTN(pData.pageREQ.mapContentId.get("调出单位"), "");
			//游标切换至二级窗体
			GWCtrlWindow.windowHandles();
			//搜索并选中
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("内部单位"), "");
			GWCtrlPage.ui_C_SEARCH_COO(pData.pageREQ.mapDynamicData.get("租赁周转材料内部调拨单-调出单位"));
			GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 确定"), "");
        	//游标切换回主窗体
        	GWCtrlWindow.windowHandlePre();
        	//游标切换回母版iframe
        	...
        	//填入【调入单位】
			//点击按钮显示甲方列表二级窗体
			GWCtrlPage.ui_C_SELECT_INPUT_BTN(pData.pageREQ.mapContentId.get("调入单位"), "");
			//游标切换至二级窗体
			GWCtrlWindow.windowHandles();
			//搜索并选中
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("内部单位"), "");
			GWCtrlPage.ui_C_SEARCH_COO(pData.pageREQ.mapDynamicData.get("租赁周转材料内部调拨单-调入单位"));
			GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 确定"), "");
        	//游标切换回主窗体
			GWCtrlWindow.windowHandlePre();
        	GWCtrlFrame.ui_C_SWITCN_DEFAULT();
	...
	public void ui_C_M_DETAILS() {
		GLog.logRecord(8, "租赁周转材料内部调拨单", "res", "填写材料明细", 5000, 0, 0, "", "微软雅黑", 20, Color.RED);
		//设置等待
		...
		ui_C_ASSERT();
		ui_C_BTN_TOPMENU("保存", "");
		GWCtrlAllure.makeScreenShot("租赁周转材料内部调拨单-详情区-材料明细-功能截图");
	}
	...
	private void ui_C_SELECT_DICT() {
		...
		try {
			...
			try {
				...
				treeViewGrid3.ui_C_SELECT_TREEVIEW(pData.pageREQ.mapDynamicData.get("租赁周转材料内部调拨单-材料类型编码"));
				//材料
				GWCtrlSelect.ByValue(GWCtrlWebElementId.CN_ID.get("查询方案"), pData.pageREQ.mapDynamicData.get("租赁周转材料内部调拨单-材料编码"));
	...
	private void ui_C_CLICK_REFER_TO(String strModuelName) {
		...
		try {
			GLog.logRecord(9, "租赁周转材料内部调拨单", "res", 
					"[" + strModuelName + "]", 
			...
			try {
				...
				GWCtrlSelect.ByValue(GWCtrlWebElementId.CN_ID.get("查询方案"), pData.pageREQ.mapDynamicData.get("租赁周转材料内部调拨单-材料编码"));
				GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
				sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("租赁周转材料内部调拨单-材料编码"));
				//移入
				GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 移入"), "");
				//确认
				GWCtrlPage.ui_C_WAIT_CLICK(GWCtrlWebElementId.CN_ID.get("二级窗体 确定"), "");
	...
	private void ui_C_INPUT_REFER() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
									4, 
									pData.pageREQ.mapContentId.get("调拨数量"), 
									pData.pageREQ.mapDynamicData.get("租赁周转材料内部调拨单-调拨数量"), 
									GWCtrlWebElementId.CN_ID.get("材料明细"));
			//点击空白处确认编辑结果
			GWCtrlDivClick.ById(GWCtrlWebElementId.CN_ID.get("材料明细"));
			//释放内存，在【参数校验】步骤中会重新读取细表
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号-测试类内部使用
			pData.pageREQ.mapDynamicData.replace("租赁周转材料内部调拨单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("租赁周转材料内部调拨单-单据编号", pData.pageREQ.mapDynamicData.get("租赁周转材料内部调拨单-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("租赁周转材料内部调拨单-单据编号"));
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			String TestResult = GScene.DYNAMIC_DATA.get("租赁周转材料内部调拨单-调拨数量");
			//进场数量
			String TestCompare = (pData.gvGrid3.ui_C_GRID3_VALUE(2, 4).replaceAll(",", ""));
