...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			//填入【供应商】
			//点击按钮显示供应商列表二级窗体
			GWCtrlPage.ui_C_SELECT_INPUT_BTN(pData.pageREQ.mapContentId.get("往来单位"), "");
			//游标切换至二级窗体
			GWCtrlWindow.windowHandles();
			//搜索并选中
			GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
			sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("运杂费记录单-往来单位"));
			pData.gvGrid3 = null;
	...
	public void ui_C_M_DETAILS() {
		pData.pageREQ.MODULE_WAIT_ID = GWCtrlWebElementId.CN_ID.get("运杂费记录单-费用项明细");
		ui_C_CLICK_TAB("费用项明细");
		ui_C_NEW_COST_ITEM();
		ui_C_ASSERT();
		ui_C_BTN_TOPMENU("保存", "");
	}
	...
	private void ui_C_INPUT_DICT() {
		GLog.logRecordTime(9, "[功能]----填写费用项明细必填项");
		try {
			GWCtrlFrame.ui_C_SWITCN_ELEMENT(GWCtrlWebElementIframe.getIframe(2));
			
			//根据行列坐标确认填写材料明细的位置
			//填写【费用项名称】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("运杂费记录单-费用项明细"));
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					4, 
					pData.pageREQ.mapContentId.get("费用项名称"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("运杂费记录单-费用项明细-费用项名称")), 
					GWCtrlWebElementId.CN_ID.get("运杂费记录单-费用项明细"));
			//填写【含税单价】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("运杂费记录单-费用项明细"));
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					6, 
					pData.pageREQ.mapContentId.get("含税单价"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("运杂费记录单-费用项明细-含税单价")), 
					GWCtrlWebElementId.CN_ID.get("运杂费记录单-费用项明细"));
			//填写【数量】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("运杂费记录单-费用项明细"));
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("运杂费记录单-费用项明细-数量")), 
					GWCtrlWebElementId.CN_ID.get("运杂费记录单-费用项明细"));
			//填写【税率】
			pData.gvGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("运杂费记录单-费用项明细"));
			pData.gvGrid3.ui_C_GRID3_FILL(1, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("运杂费记录单-费用项明细-税率(%)")), 
					GWCtrlWebElementId.CN_ID.get("运杂费记录单-费用项明细"));
			
			//点击空白处确认编辑结果
			GWCtrlDivClick.ById(GWCtrlWebElementId.CN_ID.get("运杂费记录单-费用项明细"));
			...
			pData.bRES = true;
			pData.strStatus = "编辑中";
			GLog.logRecordTime(9, "[功能]----填写费用项明细必填项成功");
		}catch (Exception e){
			pData.bRES = false;
			GWCtrlException.SwtichTo(e, 1, 9, "[功能]----填写费用项必填项失败", true);
		}
		GWCtrlPage.ui_D_IFRAME_INDEX(2, "id", pData.pageREQ.MODULE_WAIT_ID);
		GTestCase.setTestCaseRst(pData.bRES);
	}
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("运杂费记录单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("运杂费记录单-单据编号", pData.pageREQ.mapDynamicData.get("运杂费记录单-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("运杂费记录单-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			if(GAssert.assertStringEqual(TestResult, TestCompare)) {
				...
				AutoTest.GLog.logRecord(9, "运杂费记录单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 18, AutoTest.GParam.GREEN_PASS);
			}else {
				pData.bRES = false;
				AutoTest.GParam.strTestResultCode = "9999";
				AutoTest.GParam.strTestResultMsg = "ERROR";
				AutoTest.GParam.gRes = "[算法]含税金额=无税金额*(1+税率)\n则无税金额=含税金额/(1+税率)" + "\n[预期无税金额]:" + TestResult  + "\n[实际无税金额]:" + TestCompare+ "\n[算法验证]----未通过";
				AutoTest.GLog.logRecord(9, "运杂费记录单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 18, Color.RED);
			}
			...
			switch(menuName){
				...
				case "删除":{
					...
					if(GWCtrlTopMenuBtn.ui_C_GET_TOPMENUBTN_TAB2_STATUS(menuName, GTestIndicators.PageShowTime, pData.pageREQ.MODULE_WAIT_ID)) {
						GWCtrlPage.ui_C_CLICK_VERIFY_YES_INDEX(2, "取消提交", pData.pageREQ.MODULE_WAIT_ID);
						if(GWCtrlTopMenuBtn.ui_C_GET_TOPMENUBTN_TAB2_STATUS(menuName, GTestIndicators.PageShowTime, pData.pageREQ.MODULE_WAIT_ID)) {
							pData.strStatus = "编辑中";
						}
					}
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("运杂费记录单-单据编号"));
					break;
				}
				default:{
					break;
