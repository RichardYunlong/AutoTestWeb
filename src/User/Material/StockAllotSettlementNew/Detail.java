...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			//填入【收料单位】
			//点击按钮显示甲方列表二级窗体
			GWCtrlPage.ui_C_CLICK_INPUT_BTN(GParam.g_Dr, "Efd_SLDW_Name", "", "span");
			//游标切换至二级窗体
			GWCtrlWindow.windowHandles();
			//搜索并选中
			GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("内部单位"), "");
			GWCtrlPage.ui_C_SEARCH_COO(pData.pageREQ.mapDynamicData.get("材料调拨结算单-收料单位"));
        	...
        	pData.bRES = true;
			pData.strStatus = "编辑中";
			GLog.logRecordTime(9, "[功能]----填写基本信息成功");
		}catch (Exception e){
			pData.bRES = false;
			GLog.logRecordTime(9, "[功能]----填写基本信息失败");
	...
	private void ui_C_SELECT_STOCK() {
		...
		try {
			...
			try {
				...
				sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("材料库房调拨单-单据编号"));
				//确认移入
	...
	private void ui_C_INPUT_STOCK() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					5, 
					pData.pageREQ.mapContentId.get("结算单价"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料调拨结算单-调拨材料-结算单价")), 
	...
	private void ui_C_PARAMS() {
		...
		try {
        	//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("材料调拨结算单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("材料调拨结算单-单据编号", pData.pageREQ.mapDynamicData.get("材料调拨结算单-单据编号"));
			
			GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("材料调拨结算单-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			//调拨数量
			double ac1 = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 4).replaceAll(",", "")));
			//结算单价
			double dc2 = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 5).replaceAll(",", "")));
			//结算数量
			double ac2 = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 6).replaceAll(",", "")));
			//价差金额计算值
			double rc = dc1 * ac1 - dc2 * ac2;
			...
			//结算金额显示值
			String TestCompare = (pData.gvGrid3.ui_C_GRID3_VALUE(2, 10).replaceAll(",", ""));
			...
			if(GAssert.assertStringEqual(TestResult, TestCompare)) {
				...
				AutoTest.GParam.gRes = "[算法]结算金额=调拨单价*调拨数量-结算单价*结算数量" + 
						"\n[价差金额]:" + TestCompare + "\n[算法验证]----通过";
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("材料调拨结算单-单据编号"));
					break;
				}
				default:{
					break;
