...
public class Detail {
	...
	private void ui_C_SELECT_DICT() {
		...
		try {
			...
			try {
				...
				GWCtrlInputFill.ByIdUnClear(GWCtrlWebElementId.CN_ID.get("类别树-搜索区-搜索框"), pData.pageREQ.mapDynamicData.get("材料甲供计划-材料类型编码"));
				GWCtrlWait.ViewWaitingTextById(GTestIndicators.PageShowTime, GWCtrlWebElementId.CN_ID.get("类别树"), pData.pageREQ.mapDynamicData.get("材料甲供计划-材料类型编码"));
				
				//材料
				GWCtrlSelect.ByValue(GWCtrlWebElementId.CN_ID.get("查询方案"), pData.pageREQ.mapDynamicData.get("材料甲供计划-材料编码"));
				GWCtrlGrid3 sourceGrid3 = new GWCtrlGrid3("id", GWCtrlWebElementId.CN_ID.get("数据源"));
				sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("材料甲供计划-材料编码"));
				//移入
	...
	private void ui_C_INPUT_DICT() {
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					...
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料甲供计划-本期计划量")), 
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("材料甲供计划-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("材料甲供计划-单据编号", pData.pageREQ.mapDynamicData.get("材料甲供计划-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("材料甲供计划-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			Integer dc = Integer.valueOf(pData.pageREQ.mapDynamicData.get("材料甲供计划-本期计划量"));
			...
			if(GAssert.assertIntegerEqual(dc, rc)) {
				...
				AutoTest.GLog.logRecord(9, "材料甲供计划", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, AutoTest.GParam.GREEN_PASS);
			}else {
				pData.bRES = false;
				AutoTest.GParam.strTestResultCode = "9999";
				AutoTest.GParam.strTestResultMsg = "ERROR";
				AutoTest.GParam.gRes = "[算法]\n[计划数量]:" + String.valueOf(dc) + "\n[单据填写]:" + String.valueOf(rc) + "\n[算法验证]----未通过";
				AutoTest.GLog.logRecord(9, "材料甲供计划", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, Color.RED);
			}
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("材料甲供计划-单据编号"));
					break;
				}
				default:{
					break;
