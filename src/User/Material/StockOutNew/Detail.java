...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			//填入【领料单位】
			//点击按钮显示甲方列表二级窗体
			GWCtrlPage.ui_C_CLICK_INPUT_BTN(GParam.g_Dr, "Efd_LLDW_Name", "", "span");
			//游标切换至二级窗体
			GWCtrlWindow.windowHandles();
            //判断当前层级因为公司层没有内部单位按钮
            switch (GScene.DYNAMIC_DATA.get("机构层级")) {                
              case "项目部":
                GWCtrlPage.ui_C_WAIT_CLICK(pData.pageREQ.mapTopTabId.get("内部单位"), "");
                GWCtrlPage.ui_C_SEARCH_COO(pData.pageREQ.mapDynamicData.get("材料出库单-领料单位"));
	...
	private void ui_C_INPUT_STOCK() {
		GLog.logRecordTime(9, "[功能]----填写出库材料必填项");
		...
		try {
			...
			pData.gvGrid3.ui_C_GRID3_FILL(2, 
					5, 
					pData.pageREQ.mapContentId.get("出库数量"), 
					String.valueOf(pData.pageREQ.mapDynamicData.get("材料出库单-材料明细-数量")), 
			...
			pData.bRES = true;
			pData.strStatus = "编辑中";
			GLog.logRecordTime(9, "[功能]----出库材料必填项填写成功");
		}catch (Exception e){
			pData.bRES = false;
			GLog.logRecordTime(9, "[功能]----出库材料必填项填写失败");
			if (AutoTest.GTestCase.dTSSTYLE == 1)
				AutoTest.GParam.gRes = GException.getExceptionAllinformation(e);
			e.printStackTrace();
		}
	...
	private void ui_PARAMS() {
		...
		try {
        	//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("材料出库单-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("材料出库单-单据编号", pData.pageREQ.mapDynamicData.get("材料出库单-单据编号"));

			GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("材料出库单-单据编号"));
		}catch (Exception e){
	...
	private void ui_C_ASSERT(){
		...
		try {
			...
			//对应入库含税单价
			double dc = Double.valueOf((pData.gvGrid3.ui_C_GRID3_VALUE(2, 9).replaceAll(",", "")));
			//出库数量
			...
			if(GAssert.assertStringEqual(TestResult, TestCompare)) {
				...
				AutoTest.GParam.gRes = "[算法]出库含税金额=对应入库含税单价*出库数量" + 
						"\n[出库含税金额]:" + TestCompare + "\n[算法验证]----通过";
				AutoTest.GLog.logRecord(9, "材料出库单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, AutoTest.GParam.GREEN_PASS);
			}else {
				pData.bRES = false;
				...
				AutoTest.GParam.gRes = "[算法]出库含税金额=对应入库含税单价*出库数量" + 
						"\n[出库含税金额]:" + TestCompare + "\n[算法验证]----未通过";
				AutoTest.GLog.logRecord(9, "材料出库单", "res", AutoTest.GParam.gRes, 5000, 0, 0, "", "微软雅黑", 20, Color.RED);
			}
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("材料出库单-单据编号"));
					break;
				}
				default:{
					break;
