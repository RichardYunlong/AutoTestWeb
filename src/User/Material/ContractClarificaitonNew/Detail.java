...
public class Detail {
	...
	private void ui_C_INPUT_BASICINFO() {
		...
		try {
			...
			sourceGrid3.ui_C_SELECT_SOURCE(pData.pageREQ.mapDynamicData.get("材料采购合同-合同编号"));
			pData.gvGrid3 = null;
	...
	private void ui_C_PARAMS() {
		...
		try {
			//提取公共参数-单据编号
			pData.pageREQ.mapDynamicData.replace("物资合同交底-单据编号", GWCtrlPage.ui_C_GET_CODE(pData.pageREQ.MODULE_NAME));
			//提取公共参数-单据编号-全局使用
			GScene.DYNAMIC_DATA.replace("物资合同交底-单据编号", pData.pageREQ.mapDynamicData.get("物资合同交底-单据编号"));
        	
        	GLog.logRecordTime(9, "[提取公共参数]----成功----自动生成单据编号：" + pData.pageREQ.mapDynamicData.get("物资合同交底-单据编号"));
		}catch (Exception e){
	...
	public void ui_C_TAB_TOP(String menuName){
		...
		try {
			switch(menuName){
				case "关_闭":{
					GWCtrlTopTab.ui_C_CLOSE_TOPTAB(pData.pageREQ.MODULE_NAME_NEW + pData.pageREQ.mapDynamicData.get("物资合同交底-单据编号"));
					break;
				}
				default:{
					break;
